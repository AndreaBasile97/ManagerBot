...
def read_events_by_id(db, id):
    ...
    with db.cursor() as cursor:
        ...
        for evento in result:
            ...
            ev.orario_inizio = datetime.strptime(str(evento.get('data_inizio')), "%Y-%m-%d %H:%M:%S").strftime("%H:%M")
            ev.orario_fine = datetime.strptime(str(evento.get('data_fine')), "%Y-%m-%d %H:%M:%S").strftime("%H:%M")
